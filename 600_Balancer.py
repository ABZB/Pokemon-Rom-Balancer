...
def manipulate(personal, pokemon, base_formes, start_offset, offset, second_offset, gen_number):
	...
	while True:
		...
		stat_arr[stats_counted] = personal[pointer]		
		...
		if(stats_counted == 6):
			...
			#currently pointing the 6th stat, need to start from the 1rst again, so subtract 5 from the pointer
			pointer -= 5
			stat_number = 0
			
			while True:
				personal[pointer] = temp[stat_number]
			...
			if(dex_number == 486):
				personal[pointer + 19] = 125
				personal[pointer + 1 + 19] = 89
				personal[pointer + 2 + 19] = 125
			...
			#jump to the start of the next Pokemon, (offset is the length of a data block, less 5 because we are pointing to the 6th byte in that block)
			pointer += offset - 5
